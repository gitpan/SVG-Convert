...
our $STYLE_FILE = file(__FILE__)->dir->file('svg2xaml.xsl')->stringify;
...
sub convert_string {
    ...
    return $self->_convert($src_doc);
}

=head2 convert_doc($src_doc)
...
sub convert_doc {
    ...
    return $self->parser->parse_string(
        $self->convert_string($src_doc)
    );
}

=head2 convert_file($src_doc, $out_file)
